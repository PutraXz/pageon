<?php
    ...
    if(isset($_POST['delete-petugas'])){
        $id = $_POST['id'];
        $del = $conn->query("delete from siswa where nisn=$id");
    }
?>
                    ...
                    <div class="d-flex justify-content-center" style="justify-items: center;">
                        <button href="" class="btn btn-primary btn-sm mx-2" data-bs-toggle="modal" data-bs-target="#modal<?= $d3['username']?>" style="width:30px"><i class='bx bxs-edit'></i></button>
                            <form action="" method="post" id="del-form">
                                ...
                                <button class="btn btn-danger btn-sm" style="width:30px" type="submit" name="delete-petugas" onclick="deleteForm(<?= $d3['id_petugas']?>)"><i class='bx bx-trash'></i></button>
                            </form>
                        </div>
                </td>
            </tr>
    ...
    function deleteForm($id){
        ...
        confirmButtonText: 'Yes, delete it!'
            ...
            if(result.isConfirmed){
                $.ajax({
                    url: `index.php?data=data-petugas/${id}`,
                    type: GET,
                    dataType: 'json',
                    beforeSend: function(){
                            wal.fire({
